...
import { MapPin, Thermometer, Wind, Gauge, CloudRain, RefreshCw, Search } from 'lucide-react';
...
// Enhanced Earth Component with Live NASA GIBS imagery
function Earth({ rotationSpeed, selectedLayer }: { rotationSpeed: number; selectedLayer: string }) {
  ...
  // Live NASA GIBS layer mappings
  const gibsLayers = {
    'Visible Earth': 'VIIRS_SNPP_CorrectedReflectance_TrueColor',
    'Air Temperature': 'MODIS_Terra_Land_Surface_Temp_Day',
    'Visible Light': 'MODIS_Terra_CorrectedReflectance_TrueColor',
    'Infrared': 'MODIS_Terra_CorrectedReflectance_Bands367'
  };
  
  useEffect(() => {
    const loadLiveSatelliteImagery = async () => {
      setLoading(true);
      setUsingFallback(false);
      console.log(`🛰️ Loading live ${selectedLayer} imagery from NASA GIBS...`);
      
      const loader = new THREE.TextureLoader();
      const gibsLayer = gibsLayers[selectedLayer as keyof typeof gibsLayers] || gibsLayers['Visible Earth'];
      
      try {
        // Get today's date for live imagery
        const today = new Date();
        const yesterday = new Date(today);
        yesterday.setDate(yesterday.getDate() - 1);
        const dateStr = yesterday.toISOString().split('T')[0];
        
        // Build comprehensive tile mosaic from GIBS
        const canvas = document.createElement('canvas');
        canvas.width = 2048;
        canvas.height = 1024;
        const ctx = canvas.getContext('2d')!;
        
        // Fill with space/ocean background
        ...
        console.log(`📡 Fetching ${tilesX * tilesY} live satellite tiles...`);
        ...
        for (let x = 0; x < tilesX; x++) {
          for (let y = 0; y < tilesY; y++) {
            // NASA GIBS tile URL for live satellite imagery
            const tileUrl = `https://gibs.earthdata.nasa.gov/wmts/epsg4326/best/${gibsLayer}/default/${dateStr}/250m/${zoomLevel}/${y}/${x}.jpeg`;
            ...
            const promise = new Promise((resolve) => {
              ...
              img.onload = () => {
                const tileWidth = 2048 / tilesX;
                const tileHeight = 1024 / tilesY;
                ctx.drawImage(img, x * tileWidth, y * tileHeight, tileWidth, tileHeight);
                loadedCount++;
                console.log(`✅ Loaded tile ${loadedCount}/${tilesX * tilesY}`);
                resolve(null);
              };
              
              img.onerror = () => {
                // If GIBS tile fails, try fallback
                console.warn(`⚠️ GIBS tile failed: ${x},${y}`);
                resolve(null);
              };
        ...
        if (loadedCount > 0) {
          ...
          liveTexture.wrapS = THREE.RepeatWrapping;
          ...
          setLoading(false);
          console.log(`🌍 Live satellite imagery loaded: ${loadedCount} tiles`);
        } else {
          throw new Error('No tiles loaded');
        }
        
      } catch (error) {
        console.error('Failed to load live GIBS imagery:', error);
        await loadFallbackImagery();
      }
    };
    
    // Load fallback imagery
    const loadFallbackImagery = async () => {
      console.log('🗺️ Loading fallback imagery...');
      ...
      for (const url of fallbackUrls) {
        try {
          ...
          setLoading(false);
          console.log('✅ Fallback imagery loaded');
          return;
        } catch (err) {
          console.warn('Fallback URL failed, trying next...');
        }
      }
      
      // Final fallback - create basic Earth texture
    ...
    // Start loading live satellite imagery immediately
    loadLiveSatelliteImagery();
  }, [selectedLayer]);
